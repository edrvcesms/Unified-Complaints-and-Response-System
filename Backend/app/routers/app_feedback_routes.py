...
from app.services.app_feedback_services import submit_app_feedback, get_all_app_feedback, post_incident_feedback, get_all_post_incident_feedback, get_my_resolved_incidents_feedback
...
async def read_my_resolved_incidents_feedback(request: Request, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_user)):
    return await get_my_resolved_incidents_feedback(current_user.id, db)

...
async def register(request: Request, user_data: RegisterData, db: AsyncSession = Depends(get_db)):
    try:
...
async def verify_otp(request: Request, user_data: OTPVerificationData, db: AsyncSession = Depends(get_db)):
    try:
